document.addEventListener("DOMContentLoaded", function () {
  ...
  // 5. Copy Button remove indent space
  const selection = window.getSelection().toString();
  if (selection) {
    const adjusted = selection.replace(/^\s{1}/gm, "");
    e.clipboardData.setData("text/plain", adjusted);
    e.preventDefault();
  }
  ...
  // 7. Custom CSS for button and summary
